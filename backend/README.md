...
- `GET /api/topics/internal?limit=` — dipanggil generator (bukan JWT, pakai `X-Internal-Key` sama seperti `articles/internal` di bawah) buat baca topik yang statusnya masih pending, terlama dulu (FIFO), max `limit` item (default 20).
- **Generator**: bukan script cron biasa, tapi **Routine** (scheduled trigger) Claude Code yang jalan tiap malam ~jam 4 pagi WIB. Firing-nya membangunkan sesi Claude yang pertama-tama cek `GET /api/topics/internal` — kalau ada topik pending, salah satunya (biasanya yang paling lama nunggu) WAJIB dipakai jadi bahan artikel hari itu; riset internet tetap dilakukan tapi buat memperkaya/menjawab topik itu, bukan buat cari topik lain. Kalau antreannya kosong, baru generator riset bebas ke konten literasi keuangan yang lagi viral/trending di Indonesia (tema "dari mana duit datang / cara duit bekerja"). Artikelnya ditulis (600-900 kata, struktur pembuka/penjelasan/analogi/perhitungan Rupiah/insight, plus 2-4 sumber referensi nyata), lalu hasilnya dikirim ke:
  - `POST /api/articles/internal` — `{ title, bodyMd, topicId?, sources?: { title, url }[] }`, diautentikasi pakai header `X-Internal-Key: <INTERNAL_API_KEY>` (bukan JWT user, karena tidak ada manusia yang login saat Routine jalan). `topicId` diisi kalau artikelnya berasal dari salah satu topik di antrean — backend akan validasi id-nya ada, menyimpan relasi `articles.topic_id`, dan otomatis set `topic_queue.used_at` (topiknya lewat langsung berstatus "Selesai" di halaman Topik Artikel, gak perlu tandai manual lagi). Idempotent: menolak dengan `409` kalau sudah ada artikel yang dibuat hari ini (WIB). Endpoint ini selalu menolak request kalau `INTERNAL_API_KEY` belum diisi di `.env` (fail closed).
